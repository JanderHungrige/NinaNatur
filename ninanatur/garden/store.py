...
def add_obstacle(conn: sqlite3.Connection, garden_id: int, obstacle: ObstacleInput) -> int:
    cursor = conn.execute(
        "INSERT INTO obstacle (garden_id, kind, x, y, shape, width, depth,"
        " rotation, points, height, label, height_source)"
        " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (garden_id, obstacle.kind, obstacle.x, obstacle.y, obstacle.shape,
         obstacle.width, obstacle.depth, obstacle.rotation,
         None if obstacle.points is None else json.dumps(obstacle.points),
...
def _load(conn: sqlite3.Connection, row: sqlite3.Row | None) -> Garden | None:
    ...
    obstacles = [
        Obstacle(
            ...
            x=r["x"], y=r["y"], shape=r["shape"], width=r["width"],
            depth=r["depth"], rotation=r["rotation"],
            points=None if r["points"] is None else json.loads(r["points"]),
