...
Hd = H·Kd (`solar.climate`), the direct share the cell's sun hours over open
ground's, and the sky share its sky-view factor (`solar.sky`). Open ground has
...
from ninanatur.solar.raster import Directions, Moments, Part, point_sums
...
class SkyLight:
    ...
    sky: np.ndarray
    #: Relative illuminance over the months sampled, 0–1.
    relative: np.ndarray
...
def _mix(sums: np.ndarray, skies: dict[bool, np.ndarray], shown: bool, moments: Moments,
         months: list[int], climate: Climate) -> SkyLight:
    """From the sun's sums by month and half, and the skies, the rest."""
    first = skies[shown]
    ...
    for i, month in enumerate(months):
        ...
        direct_share = hours / possible if possible else np.zeros(hours.shape)
